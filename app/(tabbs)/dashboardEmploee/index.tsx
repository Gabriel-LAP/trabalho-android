  ...
  const handleCancelService = useMemo(() => {
    return async (serviceId: Service["_id"]) => {
      try {
        ...
        if (serviceToUpdate) {
          const updatedService: Service = { ...serviceToUpdate, status: 'Concluido' };
        ...
        console.error('Erro ao cancelar o servico:', error);
      }
    };
  }, [serviceList, handleService]);

  const ServiceItem = ({ item }: { item: any }) => {
    if (item.professional._id !== user?._id) return null;
    ...
    return (
      <View style={styles.service}>
        ...
        <Text style={styles.customer}>{item.customer.name}</Text>
        {/* <Text style={styles.professional}>{item.professional.name}</Text> */}
        <Text style={{color: '#fff'}}>Servi o:</Text>
        <Text style={styles.serviceType}>{item.typeOfService.name}</Text>
        <Text style={{color: '#fff'}}>Valor:</Text>
        <Text style={styles.price}>R$ {priceFormate(item.typeOfService.price)}</Text>
  ...
  return (
    <View style={styles.container}>
      {serviceList.length === 0 ? (
        <Text>Nenhum servico agendado</Text>
      ) : (
        <FlatList
          data={serviceList}
